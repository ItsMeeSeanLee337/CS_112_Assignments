...
public class Friends {
		...
		while (queue.isEmpty() == false) 
		{
			...
			while (neighbor != null) 
			{
				if (visited[neighbor.fnum] == false) 
				{
					...
					visitedAlready[neighbor.fnum] = person; 
					...
					if (g.members[neighbor.fnum].name.equals(p2)) 
					{
						...
						while (person.name.equals(p1) == false) 
						{
							...
							person = visitedAlready[g.map.get(person.name)];
						}
	...
	{		
		...
		boolean[] visited = new boolean[g.members.length];
		return breadthFirstSearch(g, g.members[0], listOfCliques, visited, school);
	}

	private static ArrayList<ArrayList<String>> breadthFirstSearch(Graph g, Person start, ArrayList<ArrayList<String>> listOfCliques, boolean[] visited, String school)
	{
		...
		if (start.school == null || start.school.equals(school) == false) 
		{
			...
			for (int j = 0; j < visited.length; j++) 
			{
				if (visited[j] == false) 
				{
					return breadthFirstSearch(g, g.members[j], listOfCliques, visited, school);
				}
			}
		}
		while (queue.isEmpty() == false) 
		{
			...
			neighbor = person.first;
			cliquesResults.add(person.name);
			while (neighbor != null) 
			{
				if (visited[neighbor.fnum] == false) 
				{
		...
		for (int i = 0; i < visited.length; i++) 
		{
			if (visited[i] == false) 
			{
				return breadthFirstSearch(g, g.members[i], listOfCliques, visited, school);
			}
		}
	...
	public static ArrayList<String> connectors(Graph g) 
	{
		...
		for (int i = 0; i < g.members.length; i++)
		{
			if (visited[i] == false) 
			{
				connectors = depthFirstSearch(connectors, g, g.members[i], visited, new int[] {0,0}, numbersOfDFS, before, predecessor, true);
			}
		}
		return connectors;
	}

	private static ArrayList<String> depthFirstSearch(ArrayList<String> connectors, Graph g, Person start, boolean[] visited, int[] count, int[] numbersOfDFS, int[] back, ArrayList<String> backward, boolean started)
	{
		visited[g.map.get(start.name)] = true;
		...
		while (neighbor != null) 
		{
			if (visited[neighbor.fnum] == false) 
			{
				count[0]++;
				count[1]++;
				connectors = depthFirstSearch(connectors, g, g.members[neighbor.fnum], visited, count, numbersOfDFS, back, backward, false);
